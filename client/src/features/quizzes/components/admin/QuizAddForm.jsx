...
const QuizAddForm = ({ subjectId, closeModal }) => {
	...
	const handleSubmit = (values) => {
		...
		dispatch(createQuiz({ formData }))
			.unwrap()
			.then(() => {
				handleSuccess(SUCCESS_MESSAGES.QUIZ_ADDED)
					...
					<div className="custom-scrollbar max-h-[450px] overflow-y-auto px-2 pb-3">
						<div className="mt-7">
							<div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-5">
								<div>
									<ImageField id="quiz-add-image" onChange={setFileImage} />
								</div>
								<div>
									<Select
										id="quiz-add-subject-id"
										name="subjectId"
										label="Môn học"
										isRequire={true}
