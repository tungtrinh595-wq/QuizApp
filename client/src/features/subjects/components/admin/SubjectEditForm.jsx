...
import { editSubject } from '@/features'
import { Input, Button, ImageField } from '@/components'

const SubjectEditForm = ({ subject, closeModal }) => {
	...
	const handleSubmit = (values) => {
		...
		dispatch(editSubject({ id: subject.id, formData }))
			.unwrap()
			.then(() => {
				handleSuccess(SUCCESS_MESSAGES.SUBJECT_EDITED)
					...
					<div className="custom-scrollbar max-h-[450px] overflow-y-auto px-2 pb-3">
						<div className="mt-7">
							<div className="grid grid-cols-1 gap-x-6 gap-y-5">
								<div>
									<ImageField
										id="subject-edit-image"
										initImage={subject.image.url}
										onChange={setFileImage}
									/>
								</div>
								<div>
									<Input
										id="subject-edit-title"
										name="title"
										label="Tên môn học"
										isRequire={true}
