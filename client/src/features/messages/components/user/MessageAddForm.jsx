...
import { createMessage } from '@/features'
import { Button, TextArea } from '@/components'

const MessageAddForm = ({ lessonId, parentMessageId = null, thread = [], closeForm }) => {
	const dispatch = useDispatch()
	const auth = useSelector((state) => state.auth)
	const messages = useSelector((state) => state.messages)
	const lessonMessages = lessonId ? messages.lessonMap?.[lessonId]?.messages : []

	const handleSubmit = (values, { resetForm }) => {
		if (!!values.text.trim()) {
			dispatch(createMessage({ lessonId, formData: { ...values, lessonId, parentMessageId }, thread }))
				.unwrap()
				.then(() => {
					resetForm()
	...
	return (
		<div className="grid grid-cols-[auto_1fr] grid-rows-[auto_auto] md:grid-rows-[auto] gap-x-3 gap-y-1 items-start w-full">
			...
			<Formik initialValues={{ text: '' }} validationSchema={messageSchema} onSubmit={handleSubmit}>
				{({ values, errors, touched, handleChange, handleBlur }) => (
					<Form className="col-span-2 md:col-span-1 text-gray-800">
						...
						<div className="flex items-center gap-3 mt-2 justify-end">
							...
							<Button
								size="md"
								disabled={lessonMessages?.isLoading || !values.text.trim()}
								type="submit"
								disableText={lessonMessages?.isLoading ? 'Đang gửi bình luận' : 'Bình luận'}
							>
								Bình luận
							</Button>
						</div>
